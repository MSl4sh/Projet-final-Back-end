...
const publicationController = {
    ...
    createPublication : async (req, res)=> {
        ...
        if(req.file){
            ...
            //         `public/uploads/publication/${fileName}`//Url du dossier de stockage des images de publication.
            //     )
            // );
            const stream = fs.createWriteStream(
                `./public/uploads/publication/${fileName}`//Url du dossier de stockage des images.
            )
    ...
    editCommentPublication : (req, res)=> {
        if (!ObjectID.isValid(req.params.id))
            return res.status(400).send('Id inconnu au bataillon: ' + req.params.id) // vérification de l'id.
    ...
    deleteCommentPublication : async (req, res)=> {
        if (!ObjectID.isValid(req.params.id))
            return res.status(400).send('Id inconnu au bataillon: ' + req.params.id) // vérification de l'id.
